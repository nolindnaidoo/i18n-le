...
//! filesystem.
//!
//! The pure modules never do — that is the point of the split — so this
//! is only ever used by `identify` and `scan`.
//!
...
impl TempTree {
    pub(crate) fn new(name: &str) -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("i18n-le-{name}-{}-{unique}", std::process::id()));
